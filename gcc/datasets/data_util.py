...
def batcher():
    def batcher_dev(batch):
        graph_q, graph_k, node_idx = zip(*batch)
        graph_q, graph_k = dgl.batch(graph_q), dgl.batch(graph_k)
        return graph_q, graph_k, node_idx
...
class NewComDataEdgelist(object):
    def __init__(self, root, name):
        print("using new data")
        self.name = name
        tmp = np.load('npz/' + name)
        mat = csr_matrix((tmp.f.adj_data, tmp.f.adj_indices, tmp.f.adj_indptr),
                         shape=(tmp.f.labels.shape[0], tmp.f.labels.shape[0])).toarray()
        g = nx.from_numpy_matrix(mat)

        label = torch.LongTensor(tmp.f.labels)
        pyg_G = from_networkx(g)
        edge_index = torch.tensor(pyg_G.edge_index, dtype=torch.long)
        label = torch.unsqueeze(torch.LongTensor(label), dim=1)

        tmp1 = torch.unique(label)
        class_num = tmp1.shape[0]
...
class NewPKLDataEdgelist(object):
    def __init__(self, root, name):
        print("using new data")
        self.name = name
...
class NewPolDataEdgelist(object):
    def __init__(self, root, name):
        print("using new data")
        self.name = name
        label_list = []
        ...
        for i in list(G.nodes()):
            label_list.append(G.nodes()[i]["value"])
            if i not in list(dic_tmp.keys()):
                dic_tmp[i] = idx
                idx = idx + 1

        # a_tmp = [i for i in range(1490)]
        # tmp_G.add_nodes_from(a_tmp)

        for j in list(G.edges()):
            (u, v) = j
            tmp_G.add_edge(dic_tmp[u], dic_tmp[v])
        adj_lists = nx.to_dict_of_lists(tmp_G)
        pyg_G = from_networkx(tmp_G)
        edge_index = torch.tensor(pyg_G.edge_index, dtype=torch.long)
...
class NewRepDataEdgelist(object):
    def __init__(self, root, name):
        print("using new data")
        self.name = name
        graph_name = name
        ...
        g = nx.Graph()
        label_list = []
        with open("./dataset/" + graph_name + "/" + graph_name + '.node_labels', 'r') as f:
        ...
        with open('./dataset/' + graph_name + "/" + graph_name + '.edges', 'r') as f:
            for line in f.readlines():
                if graph_name in ["DD68", "DD242", "DD497", "DD687"]:
                    k = line.split(" ")
                else:
...
class NewYouDataEdgelist(object):
    def __init__(self, root, name):
        ...
        node_dic = {}
        g = nx.Graph()
        node_list = [i for i in range(1138499)]
        g.add_nodes_from(node_list)
        edge_path = "YouTube-dataset/data/edges.csv"
        with open(edge_path, 'r') as f:
            for line in f.readlines():
                k = line.split(",")
                g.add_edge(int(k[0]) - 1, int(k[1]) - 1)
        label = np.zeros([1138499])
        label_path = "YouTube-dataset/data/group-edges.csv"
        with open(label_path, 'r') as f:
            for line in f.readlines():
                k = line.split(",")
                label[int(k[0]) - 1] = int(k[1]) - 1
        pyg_G = from_networkx(g)
        edge_index = torch.tensor(pyg_G.edge_index, dtype=torch.long)
        label = torch.unsqueeze(torch.LongTensor(label), dim=1)
        tmp = torch.unique(label)
...
def create_node_classification_dataset(dataset_name):
    ...
    elif dataset_name in ["cora", "citeseer", "pubmed", "squirrel", "texas", "cornell", "wisconsin", "chameleon"]:
        return NewDataEdgelist(
            "./",
            dataset_name,
        )
    elif dataset_name in ["DD242", "DD68", "DD687", "gene", "TerroristRel"]:
        return NewRepDataEdgelist(
            "./",
            dataset_name,
        )
    elif dataset_name in ["cora_full", "computer", "photo", "cs", "phy"]:
        return NewComDataEdgelist(
            "./",
            {
                "computer": "amazon_electronics_computers.npz",
                "photo": "amazon_electronics_photo.npz",
                "cora_full": "cora_full.npz",
                "cs": "ms_academic_cs.npz",
                "phy": "ms_academic_phy.npz",
            }[dataset_name],
        )
    elif dataset_name in ["kdd", "icdm", "sigir", "cikm", "sigmod", "icde"]:
        return SSSingleDataset("./panther/", dataset_name)
    else:
